 ...
 */
function corporate_x_form_install_configure_form_alter(&$form, $form_state) {
  // Pre-populate the site name with the server name.
  $form['site_information']['site_name']['#default_value'] = $_SERVER['SERVER_NAME'];
  $form['#submit'][] = 'corporate_x_form_submit';
}


function corporate_x_form_submit($form, &$form_state) {
  // Import database corporate_x file.
  $corporate_x_file = dirname(__FILE__) . '/corporate_x.sql';
  $success = import_corporate_x($corporate_x_file);
...
/**
 * Imports a database corporate_x file.
 *
 * @see demo_reset().
 */
function import_corporate_x($filename) {

  global $databases;
  $db_prefix = $databases["default"]["default"]["prefix"];
  
  // Open corporate_x file.
  if (!file_exists($filename) || !($fp = fopen($filename, 'r'))) {
    drupal_set_message(t('Unable to open corporate_x file %filename.', array('%filename' => $filename)), 'error');
    return FALSE;
  }
  ...
  // Load data from corporate_x file.
  $success = TRUE;
  $query = '';
  $new_line = TRUE;
